...
def register():
    ...
    Label(screen1, text="Please enter your row below").pack()
    ...
    Label(screen1, text="mobile *").pack()
    mobile_entry = Entry(screen1, textvariable=mobile)
    mobile_entry.pack()
...
def register_user():
    ...
    if os.path.isfile(dir_path + "/user_database/" +
        ...
        connection = sqlite3.connect("user_database/userkeys")
        createtable = "CREATE TABLE IF NOT EXISTS 'USERKEYS' ('PUBLICKEY' TEXT NOT NULL UNIQUE,'USERNAME' TEXT NOT NULL UNIQUE,PRIMARY KEY('PUBLICKEY'));"
        executeQuery(connection, createtable)
        insertkeys = "INSERT INTO userkeys(publickey, username) VALUES('"+public_key+"','"+username_info+"')"
        executeQuery(connection, insertkeys)
        connection.close()
        ...
        connection = sqlite3.connect("user_database/"+username_info+"_Wallet")
        createtable = "CREATE TABLE IF NOT EXISTS "+username_info+"_DETAILS ('USERNAME' TEXT NOT NULL UNIQUE, 'PASSWORD' TEXT NOT NULL UNIQUE, 'PUBLICKEY' TEXT NOT NULL UNIQUE, 'PRIVATEKEY' TEXT NOT NULL UNIQUE,'EMAIL' TEXT NOT NULL UNIQUE, 'MOBILE' INTEGER NOT NULL UNIQUE,PRIMARY KEY('USERNAME','PUBLICKEY'));"
        executeQuery(connection, createtable)
        insertQuery = "INSERT INTO "+username_info+"_DETAILS(USERNAME,PASSWORD,PUBLICKEY,PRIVATEKEY,EMAIL,MOBILE) VALUES('"+username_info+"','"+password_info+"','"+public_key+"','"+private_key+"','"+email_info+"','"+str(mobile_info)+"');"
        executeQuery(connection, insertQuery)

        messageTable = "CREATE TABLE IF NOT EXISTS "+username_info+"_MESSAGES ('TIMESTAMP' TEXT NOT NULL, 'MESSAGE' TEXT NOT NULL);"
        executeQuery(connection, messageTable)
        connection.close()
...
def login():
    ...
    Label(screen2, text="Please enter your row below").pack()
...
def login_check():
    ...
    if os.path.isfile(dir_path + "/user_database/" +
                      login_user_entry + "_Wallet"):
        f = open(dir_path + "/user_database/" +
                 login_user_entry + "_Wallet.txt", "r")
        data = yaml.load(f.read())
        f.close()
        connection = sqlite3.connect("user_database/"+login_user_entry+"_Wallet")
        cred = "SELECT password from "+login_user_entry+"_DETAILS;"
        result = executeQuery(connection, cred).fetchall()
        connection.close()

        if result and result[0][0] == login_password_entry:
            main_screen()
        else:
...
def view_keys():
    ...
    connection = sqlite3.connect(dir_path + "/user_database/" +
                      login_user_entry + "_Wallet")
    selectQuery = "SELECT PUBLICKEY,PRIVATEKEY FROM "+login_user_entry+"_DETAILS"
    result = executeQuery(connection,selectQuery).fetchall()[0]
    connection.close()
    print(result)
    data = {'public_key':result[0], 'private_key':result[1]}
...
def view_transactions():
    ...
    connection = sqlite3.connect("user_database/"+login_user_entry+"_Wallet")
    query = "SELECT PUBLICKEY FROM "+login_user_entry+"_DETAILS"
    result = executeQuery(connection,query).fetchall()[0]
    connection.close()
    sender_public_address = result[0]
    ...
    transaction = ""
    connection = sqlite3.connect("blockchain")
    query = "SELECT transactions,blockno,receiverpublickey,message from blocks where senderpublickey='"+sender_public_address+"';"
    result = executeQuery(connection,query).fetchall()
    for row in result:
        temp = "block - " + str(row[1]) + "\nReceiverkey - " + row[2] + "\ntimestamp - " + row[0] + "\nmessage - " + row[3] + "\n\n"
        transaction += temp

    connection.close()
    T = tk.Text(screen5, height=20, width=60)
    T.pack()
    T.insert(tk.END, str(transaction))
...
def view_message():
    ...
    connection = sqlite3.connect("user_database/"+login_user_entry+"_Wallet")
    query = "Select TIMESTAMP,MESSAGE from "+login_user_entry+"_MESSAGES"
    messages = executeQuery(connection,query).fetchall()
    connection.close()
    ...
    if not messages:
        ...
        T.pack()
        for row in messages:
            for timestamp, message in row:
                T.insert(tk.END, "["+timestamp+"] - " + message)
